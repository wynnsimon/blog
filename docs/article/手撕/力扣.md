...
二维动态规划
dp表行数是第一个字符串的长度+1，列数是第二个字符串的长度+1，第0行和第0列的值都为0表示空字符串的情况，表格中`dp[i][j]`代表第一个字符的前i个字符和第二个字符串的前j个字符的最长公共子序列长度。如果`s1[i]===s2[j]`则`dp[i][j]`赋值为`dp[i-1][j-1]+1`也就是它左上角各自最长公共子序列长度+1表示发现了新的匹配字符，如果不想等则`dp[i][j]`赋值为`max(dp[i-1][j],dp[i][j-1])`去左边或上边的最大值表示跳过当前字符，依次把整张表填完，结果即为dp表右下角的值
![](attachments/Pasted%20image%2020260223202135.png)
```ts
