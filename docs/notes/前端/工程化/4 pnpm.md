...
对于循环依赖问题，pnpm使用软链接拍平依赖，拍平的依赖是软链接。
对于依赖重复安装问题，pnpm使用硬链接，在全局拥有一个全局仓库，项目中的依赖是硬链接到全局仓库中
除此之外对于同一个依赖的不同版本的pnpm还会做额外的优化，对于本地已有的版本和项目需要的版本做diff，不同版本链接的依赖实际上是同一个版本的依赖加patch
