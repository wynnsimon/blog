...
**为什么要有反射**
1. 将Object的一些明显属于语言内部的操作（比如Object.defineProperty），放到Reflect上。现阶段某些操作同时存在Object和Reflect上，未来的新方法将只部署在Reflect上。
2. 修改某些Object函数的返回结果，让其变得更合理。比如，`Object.defineProperty(obj, name, desc)`在无法定义属性时，会抛出一个错误，而`Reflect.defineProperty(obj, name, desc)`则会返回false。
...
4. Reflect的方法与Proxy的方法一一对应，只要是Proxy的方法，就能在Reflect上找到对应的方法。这就让Proxy对象可以方便地调用对应的Reflect方法，完成默认行为。
