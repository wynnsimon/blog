...
| 方案                    | 核心机制                         | 优点                     | 缺点                               |
| :-------------------- | :--------------------------- | :--------------------- | :------------------------------- |
| npm 私包                | 发布二进制包/源码包到私有仓库，通过版本依赖引用     | 版本控制严格，依赖关系清晰，解耦性好     | 更新流程长（需发版->安装），本地调试公共代码麻烦        |
| Git Submodule/subtree | 在主仓库中嵌入独立的 Git 仓库指针          | 代码物理隔离，公共库历史独立，权限可控    | 命令复杂（init/update），容易出错，同步更新繁琐    |
| Monorepo              | 所有项目代码在同一个仓库，通过 Workspace 管理 | 开发体验最佳，修改公共代码即时生效，重构方便 | 仓库体积大，对工具链（如 Turborepo）有要求，权限难隔离 |
| 微前端/模块联邦              | 架构模式，通常结合上述方案（如联邦或iframe）    | 技术栈无关，独立部署，团队自治        | 主要是架构层面的，本身不解决代码复用，需配合其他方案       |
